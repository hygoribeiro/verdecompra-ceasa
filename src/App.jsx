...
function Login() {
  ...
  return <main className="auth-page"><section className="auth-card"><div className="brand auth-brand"><img className="brand-logo" src="/logo-mark.png" alt="Verdurão Ribeiro" width="48" height="48" /><div><strong>VERDURÃO RIBEIRO</strong><span>Controle CEASA</span></div></div><p className="eyebrow">Acesso seguro</p><h1>Entrar</h1><p className="muted">Use o e-mail e a senha fornecidos pelo administrador.</p><form onSubmit={submit}><label>E-mail<input type="email" required value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} /></label><label>Senha<input type="password" minLength="6" required value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} /></label><button className="btn primary full">Entrar</button></form>{message && <p className="auth-message">{message}</p>}</section></main>;
}
